...
class Charity_EM_Widget extends WP_Widget {
	
	var $defaults;
	
    /** constructor */
    function Charity_EM_Widget() {
    	$this->defaults = array(
    		'title' => __('Charity Events','charity'),
    		'scope' => 'future',
    		'order' => 'ASC',
    		'limit' => 5,
    		'category' => 0,
			...
			'all_events' => 0,
			'all_events_text' => __('all events', 'charity'),
			'no_events_text' => '<li>'.__('No events', 'charity').'</li>'
    	);
		$this->em_orderby_options = apply_filters('em_settings_events_default_orderby_ddm', array(
			'event_start_date,event_start_time,event_name' => __('start date, start time, event name','charity'),
			'event_name,event_start_date,event_start_time' => __('name, start date, start time','charity'),
			'event_name,event_end_date,event_end_time' => __('name, end date, end time','charity'),
			'event_end_date,event_end_time,event_name' => __('end date, end time, event name','charity'),
		)); 
    	$widget_ops = array('description' => __( "Display a list of events on Events Manager.", 'charity') );
        parent::WP_Widget(false, $name = 'Charity Events', $widget_ops);	
    }
    ...
    function form($instance) {
        ...
        ?>
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php esc_html_e('Title', 'charity'); ?>: </label>
			<input type="text" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo esc_attr($instance['title']); ?>" class="widefat" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('limit'); ?>"><?php esc_html_e('Number of events','charity'); ?>: </label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('scope'); ?>"><?php esc_html_e('Scope','charity'); ?>: </label><br/>
		...
		<p>
			<label for="<?php echo $this->get_field_id('order'); ?>"><?php esc_html_e('Order By','charity'); ?>: </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('order'); ?>"><?php esc_html_e('Order','charity'); ?>: </label>
			<select id="<?php echo $this->get_field_id('order'); ?>" name="<?php echo $this->get_field_name('order'); ?>" class="widefat">
				<?php 
				$order_options = apply_filters('em_widget_order_ddm', array(
					'ASC' => __('Ascending','charity'),
					'DESC' => __('Descending','charity')
				)); 
				?>
		...
		<p>
            <label for="<?php echo $this->get_field_id('category'); ?>"><?php esc_html_e('Category IDs','charity'); ?>: </label>
            ...
            <em><?php esc_html_e('1,2,3 or 2 (0 = all)','charity'); ?> </em>
        </p>
        <p>
			<label for="<?php echo $this->get_field_id('all_events'); ?>"><?php esc_html_e('Show all events link at bottom?','charity'); ?>: </label>
		...
		<p id="<?php echo $this->get_field_id('all_events'); ?>-section">
			<label for="<?php echo $this->get_field_id('all_events'); ?>"><?php esc_html_e('All events link text?','charity'); ?>: </label>
		...
		<em><?php echo sprintf( esc_html__('The list is wrapped in a %s tag, so if an %s tag is not wrapping the formats below it will be added automatically.','charity'), '<code>&lt;ul&gt;</code>', '<code>&lt;li&gt;</code>'); ?></em>
        <p>
			<label for="<?php echo $this->get_field_id('format'); ?>"><?php esc_html_e('List item format','charity'); ?>: </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('no_events_text'); ?>"><?php _e('No events message','charity'); ?>: </label>
...
if (is_plugin_active('events-manager/events-manager.php')) :
    add_action('widgets_init', create_function('', 'return register_widget("Charity_EM_Widget");'));
endif;
