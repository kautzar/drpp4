...
function charitySetup() {
    ...
    register_nav_menus(array(
        ...
        'single_causes' => __('Single Cause', "charity"),
    ));
}
...
function charityNavMenu() {
    ...
    $single_option = vp_option('vpt_option.single_causes_mode');

    $single_location = ($single_option) ? 'single_causes' : 'header_menu';
    //var_dump($single_option);

    $withLocation = array(
        'theme_location' => $single_location,
        'menu' => '',
        'container' => 'nav',
        'container_class' => '',
        'container_id' => '',
...
function charityHomeLayout() {
    ...
    $home_mod = vp_option("vpt_option.single_causes_mode");

    if ($home_mod):
        $causes_latout = vp_option("vpt_option.causes_home_layout");
        $home_4_select = vp_option("vpt_option.charity_home_4_select");
        $home_5_select = vp_option("vpt_option.charity_home_5_select");
        switch ($causes_latout) {
            case "four":
                update_option("show_on_front", "page");
                update_option('page_on_front', $home_4_select);
                break;
            case "five":
                update_option("show_on_front", "page");
                update_option('page_on_front', $home_5_select);
                break;
            default :
                $front_page_id = get_option('page_on_front');
                $templatePage = get_post_meta($front_page_id, "_wp_page_template", true);

                if ($templatePage == "charity-home-one.php" || $templatePage == "charity-home-two.php" || $templatePage == "charity-home-three.php" || $templatePage == "charity-home-four.php" || $templatePage == "charity-home-five.php" || $templatePage == "charity-shop-landing.php") {
                    update_option("show_on_front", "posts");
                    update_option('page_on_front', "");
                }
                break;
